...
import type { AdminOrder, AdminOrderListResponse, AdminStats, AdminUser, AdminUserListResponse, AdminUserQuery, CategoryInput, ExportQuery, ProductInput, StatsQuery, UpdateOrderStatusInput } from '../types/admin';
...
export function exportCsv(kind: 'orders' | 'products', params: ExportQuery = {}): Promise<void> {
  const query = new URLSearchParams();
  if (kind === 'orders') {
...
export function useUpdateProduct() { const invalidate = useInvalidate([productsKey, ['product']]); return useMutation({ mutationFn: updateProduct, onSuccess: invalidate }); }
